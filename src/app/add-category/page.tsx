...
export default function CategoryPage() {
 ...
 const validateForm = () => {
  ...
  const containsSQLInjection = (text: string) => {
    // Only check for SQL injection if there are suspicious patterns with quotes/semicolons
    // Don't flag common words like "and", "or" in normal text
    const hasSuspiciousChars = /['";]/.test(text)
    if (!hasSuspiciousChars) return false
    
    return /(['";])+|(--)+|(\b(SELECT|INSERT|DELETE|UPDATE|DROP|UNION)\b)/i.test(text)
  }

  const isValidText = (text: string, pattern: RegExp, maxLen: number) =>
    pattern.test(text) && text.length <= maxLen

  const isValidDescription = (text: string, maxLen: number) => {
    if (text.length > maxLen) return false
    // More permissive: exclude only dangerous characters like <, >, {, }, [, ], \, `
    // Allow all other printable characters including Unicode punctuation
    return !/[<>{}[\]\\`]/.test(text)
  }
  ...
  if (parentCategory === "none" && !industry) {
    ...
    if (!isValidDescription(description.trim(), 500)) {
      toast.info("Description contains invalid characters or is too long (max 500 chars)");
      return false;
    }
  ...
  if (metaTitle) {
    ...
    if (!isValidDescription(metaTitle.trim(), 70)) {
      toast.info("Meta title contains invalid characters or is too long (max 70 chars)")
      return false
    }
  }
  ...
  if (metaDescription) {
    ...
    if (!isValidDescription(metaDescription.trim(), 160)) {
      toast.info("Meta description contains invalid characters or is too long (max 160 chars)")
      return false
    }
  }
